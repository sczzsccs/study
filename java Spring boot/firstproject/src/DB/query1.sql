...
select *from product_table
	where product_name like "%상품%" 
    order by product_price asc, product_stock desc;


select product_id, Product_name, product_price, product_stock from product_table
	where product_price >2500;

use first_db;
select product_id, Product_name, product_price, product_stock, created_at, updated_at from product_table
	order by Product_name asc;


-- product_table example InputData
-- UPDATE product_table SET product_price = 3000 where product_id = 'book';
-- insert into product_table(product_id, product_name, product_price, product_stook)
--	values('4 번째', '이름', 1300, 333);
